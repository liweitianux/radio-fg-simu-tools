...
{ 
  if (argc != 3)
    {
      cerr << "Usage:" << endl;
      cerr << "    " << argv[0] << " DM_Mass_LowerLimit dndmdata_file" << endl;
      exit(-1);
    }
	  ...
	  if(ms[j]>=lower_mass&&zs[i]<=upper_redshift)
	    {
	      ...
	      double da=calc_angular_distance(zs[i])/Mpc;
	      n+=delta_z*delta_m*fs[i][j]*(c/km)/(calc_H(zs[i])/(km/s/Mpc))*da*da*(1+zs[i])*(1+zs[i])*pow(10./180*3.1415926,2);
	    }
	}
      
      //  cout<<n/calc_H(z)*da*da*(1+z)*(1+z)*78.5<<endl;
