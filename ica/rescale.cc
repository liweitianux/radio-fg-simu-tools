  ...
  for(double r1=r+1;r1<r+3;++r1)
      ...
      for(double theta=0;theta<2.*3.1415926;theta+=.5/r1)
	{
	  ...
	  if(mask(int(di+center_i),int(dj+center_j))!=0&&iter==pix_set.end()
	     &&img(int(di+center_i),int(dj+center_j))!=0)
	    {
	      //	      cout<<"a"<<endl;
...
int main(int argc,char* argv[])
{
  ...
  if(argc!=9)
    {
      ...
      cerr<<"Usage:"<<argv[0]<<
	"<source1> <source2> <source 3> <source4> <target > <outname> <clu_num> <mask.fits>"<<endl;
      cerr<<"Example: if you have 4 images, img_1,2,3,4.fits,\n with the 2nd of which is cluster."	\
	"And you want to rescale these four images with target target.fits"\
	"you can run\n"\
	  <<argv[0]<<" img_1.fits img_2.fits img_3.fits img_4.fits target.fits out.fits 2 mask.fits"<<endl;
      return -1;
    }

  ofstream err_ofs("error.dat",ios_base::app);
  cfitsfile ff_mask;
  ...
  for(int i=1;i<=4;++i)
    {
      ...
      ff>>mx;
      if(clu_num==i&&is_negative(mx,center_i,center_j,r))
	{
	  mx*=-1;
	  //	  cout<<"negative\n";
	}
      ...
      if(clu_num==i)
	{
	  ...
	  cout<<sigma_bkg/mean(imgs.back())<<endl;
	  err_ofs<<sigma_bkg/mean(imgs.back())<<endl;
	}
    }
  //  return 0;
  //  cout<<"------"<<endl;
  ...
  cfitsfile ff_out;
  ff_out.create(argv[6]);

  //  cout<<p[clu_num]<<endl;
  

  blitz::Array<double,2> out_img(one_to_two(imgs[clu_num-1],mask));
  out_img*=p[clu_num];
