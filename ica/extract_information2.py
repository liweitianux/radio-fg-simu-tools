...
mask=pyfits.open('mask.fits')[0].data
img65=pyfits.open('65.fits')[0].data
peak_position=max_element(img65)
...
for i in range(1,rmax):
    ...
    sb_file.write(`pixel2arcmin(i)`+" "+`sb`+" "+`(max_sb-min_sb)/2`+'\n')
...
for freq in range(65,186,15):
    ...
    for i in range(peak_position[0]-rmax,peak_position[0]+rmax):
        for j in range(peak_position[1]-rmax,peak_position[1]+rmax):
            ...
            if r<rmax:
                sum+=img[i,j]
    spec_total_file.write(`freq`+' '+`Tb2flux(sum,freq)/1e-23`+' '+`Tb2flux(sum,freq)*sberror/1e-23`+'\n')
...
for freq in range(65,186,15):
    ...
    spec_peak_file.write(`freq`+' '+`kT_peak`+' '+`sberror*kT_peak`+'\n')
