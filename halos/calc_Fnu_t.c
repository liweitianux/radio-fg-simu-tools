 ...
 * at given frequency 'nu' and time 't' (different to 'calc_NN0.c')
 * by 'N0' (default: 1.0) relativistic electron(s) with initial spectral
 * index of Gamma (default: 3.0).
 *
 * The final output column flux 'F_{nu}(t)' at given frequency and
 * time is calculated with:
 *     F_{nu}(t) = j_{nu}(t) / NN0
 * where the 'NN0' is the output of 'calc_NN0.py':
 *     NN0 = j_{nu=1.4GHz}(t=0) / F_{1.4GHz}(t=0)
 *
 * Note: This tools is almost the same as 'calc_NN0.c'.
 *
 * Aaron LI
 * 2015/04/13
 *
...
main( int argc, char **argv )
{
    if ( argc != 5 ) {
        ...
        fprintf( stderr, "    %s <B_local_uG> <nu_MHz> "
              "<in: ra_dec_z_m_F1400_rcDA_T_alpha_NN0_age.dat> "
              "<out: ra_dec_z_m_F${nu}_rcDA_T_alpha.dat>\n", argv[0] );
        exit( EXIT_FAILURE );
    }

    double B_local = atof( argv[1] ) * 1e-6; /* uG -> Gauss */
    double nu      = atof( argv[2] ) * 1e6; /* MHz -> Hz */

    FILE *fdata = fopen( argv[3], "r" );
    if ( fdata == NULL ) {
    ...
    double ra, dec, z, m, F1400, rcDA, T, alpha, NN0, age;
    double b, nu_z, F_nu_t;

    int i = 0;
    while ( fscanf(fdata, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf",
                &ra, &dec, &z, &m, &F1400, &rcDA, &T, &alpha, &NN0, &age) == 10 ) {
        i++;
        printf("# %d #\n", i);
        //fprintf( stderr, "== %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg ==\n",
        //        ra, dec, z, m, F1400, rcDA, T, alpha, NN0, age );
        age *= Gyr; /* Gyr -> s */
        b = decay_rate( B_local, z );
        nu_z = nu * (1.0 + z);
        F_nu_t = emission_coef( gamma_lower, gamma_upper, N0, Gamma,
                B_local, b, age, nu_z ) / NN0;
        //fprintf( stderr, "** F_nu_t: %lg **\n", F_nu_t );
        fprintf( fout, "%.10lg %.10lg %.10lg %.10lg %.10lg %.10lg %.10lg %.10lg\n",
                ra, dec, z, m, F_nu_t, rcDA, T, alpha );
    }
...
{
    //fprintf( stderr, "DEBUG: b*gamma*age = %lg*%lg*%lg = %lg\n", b, gamma, age, b*gamma*age);
    if ( b * gamma * age >= 1.0 ) {
        return 0.0;
    } else {
